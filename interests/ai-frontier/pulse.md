...
- **2026-04-25** — Claude Code web/mobile refresh with teleport (web↔CLI handoff). GPT-5.5 topped CursorBench. Cursor disclosed -23% gross margin at $2.7B ARR — model dependency is an active liability. OpenClaude v0.6 shipped open-source. Mollick: multi-agent org design is next unsolved frontier. Levie: Jevons paradox — AI increases labor demand, doesn't shrink it.
- **2026-04-24** — GPT-5.5 launched; Mollick calls it best for hard problems, Box CTO confirms 10-pt accuracy jump on enterprise content vs 5.4. DeepSeek V4 dropped same cycle: frontier benchmarks, open-source, cheapest in tier. Model convergence thesis is now two-sided: frontier labs compete on deployment surface (Brockman pitching 5.5 for spreadsheets/slides, not just code) while open-source closes on raw capability. Claude Managed Agents persistent memory entered public beta (file-based, API-exportable) — addresses the open question about managed agents as a real category. Claude Code quality regression root-caused to Agent SDK harness bugs, not model — fixed in v2.1.116+. Codex shipped an auto-review guardian sub-agent that gates unsafe actions, defining the concrete agent oversight pattern for production systems.
