---
id: DOC-047
created: 2026-04-14
updated: 2026-04-20
type: knowledge
domain: building-out-loud
status: active
---

# Writing Queue — andrewdonato.com

> The pool of writing topics for the site's writing section. Posts publish every other day via scheduled task `publish-andrewdonato-writing`. After a post ships, its topic moves to **Published** and a new one gets added to **Backlog → Active 8** to keep the queue full.

## Guardrails (read every time before writing)

- **Nothing private.** No family details beyond what's already public, no personal finances, no health, no relationship specifics about real named people.
- **Nothing project-specific.** No MSA, TrackForce, Facts Unlocked, Knox Brothers, Sales Edge, ScrollProxy, urcooked, trend-jacker, or any current client/employer specifics. Generalize the lesson.
- **Murky → skip and document.** If a topic's framing is unclear, the angle feels forced, or the post would require info from a private source — DO NOT write it. Move it to `needs-review.md` in this folder with a one-line note about why and pick the next topic in the queue.
- **Voice.** Andrew's voice — see style notes in `areas/personal/private/book-for-ellie.md`. Conversational, story-first, one-line punches, no bullet lists in the prose, lowercase title in the file but title-case in the H1, lands a lesson plainly after earning it. ~600–1100 words. No "in conclusion." No corporate softening.
- **Tone.** Musings. Same energy as Building Out Loud — thinking out loud, not pronouncing from a mountain.

---

## Active 8 (the current pool — task picks the top one each run)

| # | Topic | Category | Notes |
|---|-------|----------|-------|
| 1 | You're Not Behind. You're Early in Something That Doesn't Have a Name Yet. | Life / work | Pep talk for builders in undefined spaces |
| 2 | Your Pipeline Is a Mirror | Sales | What your pipeline says about how you actually spend your time |
| 3 | Why "Just Following Up" Is the Worst Sentence in Sales | Sales | Pulled from backlog after seeding post #1 |
| 4 | The Four Emotional States Every Buyer Cycles Through | Sales | Pulled from backlog 2026-04-15 |
| 5 | Building in Public Is Just Keeping a Journal Where Strangers Can Read It | Building | Pulled from backlog 2026-04-16 |
| 6 | The Skills That Just Got Obsolete and the Ones That Just Got 10x More Valuable | Life / work | Pulled from backlog 2026-04-16 |
| 7 | Sovereign Individuals: What Happens When One Person Can Run a Real Business | Life / work | Pulled from backlog 2026-04-18 |
| 8 | The People You Call When Something Good Happens vs. Something Bad | Relationships | Pulled from backlog 2026-04-20 |

## Backlog (next-up to refill the Active 8)

- The People You Call When Something Good Happens vs. Something Bad
- The Case Against Optimization
- Quitting Is a Skill and Most People Are Bad at It
- What I Learned About Ambition From People Who Had None
- Why I Started Writing Again at 35
- The Salesperson Who Taught Me That Empathy Is a Competitive Advantage
- Connectors Are the Loneliest People in the Room
- Model Lock-In Was Never a Moat — added 2026-04-17 from scrollproxy surfacing; reaction to the Factory $1.5B framing. Generalize to the broader AI product strategy question: if every foundation model is swappable, what actually defends your product?
- AI Nailed Coding. Why Can't It Nail Your Job?
- Every SaaS Company Is About to Have an Identity Crisis — added 2026-04-19 from ScrollProxy surfacing; Aaron Levie's "headless enterprise" thesis + gregisenberg on how agent-native startups undercut legacy SaaS. When agents consume APIs directly, the UI layer (where SaaS monetizes) becomes optional. The business model that powered the last decade of software is getting compressed from below. No project specifics — make it a general observation about what happens when the product surface shifts from human screens to agent calls. — added 2026-04-17 from scrollproxy surfacing; the knowledge layer / context problem. Git gives every coding tool the same shared substrate; knowledge work has no equivalent. No project specifics — make it a general observation about where AI still fumbles and why.

## Published

| Date | Title | Post File | PR |
|------|-------|-----------|-----|
| 2026-04-14 | Why I Stopped Using AI to Write and Started Using It to Think | [post](posts/2026-04-14-why-i-stopped-using-ai-to-write.md) | [#1](https://github.com/AndrewDonato444/andrew-resume-app/pull/1) |
| 2026-04-15 | The Bike Ride Test | [post](posts/2026-04-15-the-bike-ride-test.md) | [#2](https://github.com/AndrewDonato444/andrew-resume-app/pull/2) |
| 2026-04-16 | Stop Selling to People Who Are Nodding | [post](posts/2026-04-16-stop-selling-to-people-who-are-nodding.md) | [#3](https://github.com/AndrewDonato444/andrew-resume-app/pull/3) |
| 2026-04-16 | The Target Keeps Moving | [post](posts/2026-04-16-the-target-keeps-moving.md) | [#4](https://github.com/AndrewDonato444/andrew-resume-app/pull/4) |
| 2026-04-18 | The Managed Agent Era Is Here and Most People Are Still Hiring Humans for It | [post](posts/2026-04-18-the-managed-agent-era-is-here.md) | [#5](https://github.com/AndrewDonato444/andrew-resume-app/pull/5) |
| 2026-04-20 | Networking Is a Word for People Who Aren't Good at Being a Friend | [post](posts/2026-04-20-networking-is-a-word.md) | [#6](https://github.com/AndrewDonato444/andrew-resume-app/pull/6) |
